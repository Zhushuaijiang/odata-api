...
namespace Yoisoft.DataBase.Oracle
{
    ...
    public class Database : IDatabase
    {
        ...
        public IEnumerable<T> GetDBTableFields<T>(string tableName) where T : class,new()
        {
            try
            {
                ...
                strSql.Append(@"SELECT
                            ...
                            ORDER BY
	                            col.column_id");
                return dbcontext.Database.Connection.Query<T>(strSql.ToString(), new { tableName = tableName.ToUpper() }, dbTransaction);
            }
            catch (Exception)
            {
        ...
        //public int ExecuteBySql(string strSql, OracleDynamicParameters oracleDynamicParameters)
        //{
        //    dbcontext.Database.Connection.Execute();
        //}
        #endregion
    }
}
