...
{
    /// <summary>
    /// 表：YY_CODE_SCHOOL 实体类   学历表
    /// </summary>
    public  class CODE_SCHOOLController : ODataController
    {
        #region  default
        /// <summary>
        /// 查询列表所有
        ...
        [HttpGet]
        [EnableQuery]
        public IHttpActionResult Get(ODataQueryOptions<CODE_SCHOOLEntity> odataQueryOptions)
        {
            Expression<Func<CODE_SCHOOLEntity, bool>> myfilter = null;
            if (odataQueryOptions.Filter != null)
            {
                myfilter = odataQueryOptions.Filter.ToExpression<CODE_SCHOOLEntity>();
            }
            try
            {
                CODE_SCHOOLService service = new CODE_SCHOOLService();
                object id;
                var expression = LinqExtensions.True<CODE_SCHOOLEntity>();
                if (myfilter != null)
                {
        ...
        /// </summary>
        /// <param name="model"></param>
        public void Post(CODE_SCHOOLEntity model)
        {
            CODE_SCHOOLService service = new CODE_SCHOOLService();
