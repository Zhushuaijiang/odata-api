...
namespace YoiEmr_Api.Controllers.Odata.Patient
{
    ...
    public class BradenScoreController : ODataController
    {
        ...
        [HttpGet]
        [EnableQuery]
        public IHttpActionResult Get(ODataQueryOptions<BradenScoreEntity> odataQueryOptions)
        {
            Expression<Func<BradenScoreEntity, bool>> myfilter = null;
            if (odataQueryOptions.Filter != null)
            {
                myfilter = odataQueryOptions.Filter.ToExpression<BradenScoreEntity>();
            }
            try
            {
                BradenScoreService service = new BradenScoreService();
                var expression = LinqExtensions.True<BradenScoreEntity>();
                if (myfilter != null)
                {
        ...
        [HttpPatch]
        public IHttpActionResult Patch([FromODataUri] string key, Delta<BradenScoreEntity> patch)
        {
            BradenScoreService service = new BradenScoreService();
            object id;
        ...
        public void Put([FromODataUri] string key, BradenScoreEntity model)
        {
            BradenScoreService service = new BradenScoreService();
        ...
        /// </summary>
        /// <param name="model"></param>
        public void Post(BradenScoreEntity model)
        {
            BradenScoreService service = new BradenScoreService();
