...
namespace YoiEmr_Api.Controllers.Odata.Patient
{
    ...
    public class CNRController : ODataController
    {
        ...
        [HttpGet]
        [EnableQuery]
        public IHttpActionResult Get(ODataQueryOptions<CNREntity> odataQueryOptions)
        {
            Expression<Func<CNREntity, bool>> myfilter = null;
            if (odataQueryOptions.Filter != null)
            {
                myfilter = odataQueryOptions.Filter.ToExpression<CNREntity>();
            }
            try
            {
                CNRService service = new CNRService();
                var expression = LinqExtensions.True<CNREntity>();
                if (myfilter != null)
                {
        ...
        [HttpPatch]
        public IHttpActionResult Patch([FromODataUri] string key, Delta<CNREntity> patch)
        {
            CNRService service = new CNRService();
            object id;
        ...
        public void Put([FromODataUri] string key, CNREntity model)
        {
            CNRService service = new CNRService();
        ...
        /// </summary>
        /// <param name="model"></param>
        public void Post(CNREntity model)
        {
            CNRService service = new CNRService();
